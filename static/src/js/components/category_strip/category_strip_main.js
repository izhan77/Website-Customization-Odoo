...
class CategoryStripComplete {
    ...
    this.scrollContainer.addEventListener(
      ...
      (e) => {
        if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) {
          ...
          this.currentTransform = Math.min(
            Math.max(this.currentTransform, maxScroll),
            0
          );
          this.applyTransform();
          this.updateArrowStates();
  ...
  scrollLeft() {
    const maxScroll = 0;
    this.smoothScrollTo(
      Math.min(this.currentTransform + this.scrollAmount, maxScroll)
    );
  }
  ...
  scrollRight() {
    ...
    const maxScroll = -(contentWidth - containerWidth);
    this.smoothScrollTo(
      Math.max(this.currentTransform - this.scrollAmount, maxScroll)
    );
  }
  ...
  createPlaceholderSection(sectionId) {
    ...
    section.innerHTML = `
            <div class="container mx-auto px-4">
                <h2 class="text-4xl font-bold mb-8 text-center text-gray-800">${this.formatSectionTitle(
                  sectionId
                )}</h2>
                <div class="text-center">
                    <p class="text-lg text-gray-600 mb-4">Menu items for ${this.formatSectionTitle(
                      sectionId
  ...
  updateActiveStateOnScroll() {
    const sections = document.querySelectorAll(
      '[data-section], .menu-section, [id*="-section"]'
    );
    ...
    if (activeSection) {
      const activeItem = document.querySelector(
        `.category-item[href="#${activeSection}"]`
      );
      if (activeItem) {
  ...
  scrollCategoryIntoView(categoryItem) {
    ...
    if (
      ...
      this.smoothScrollTo(
        Math.max(Math.min(-scrollPosition, maxScroll), minScroll)
      );
    }
  }
